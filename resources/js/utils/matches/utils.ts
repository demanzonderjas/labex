...
export function getMatchingPercentage(attempt: TExchangeAttempt, filters: TFormField[], fields: TFormField[]): number {
	const activeFilters = filters.filter(
		(filter: TFormField) =>
			filter.value != "" && !filter.hidden && fieldMeetsDependencies(filter, 0, fields) && !fieldShouldBeIgnoredInMatch(filter)
	);
	const matchingFilters: TSpecStatus[] = activeFilters.map(filter => checkIfFilterMatches(filter, attempt, filters, fields));
	const fullMatchFilters = matchingFilters.filter(status => status === TSpecStatus.Match);
	const partialMatchFilters = matchingFilters.filter(status => status === TSpecStatus.PartialMatch);
	const PARTIAL_MATCH_WEIGHT = 0.5;
	return ((fullMatchFilters.length + partialMatchFilters.length * PARTIAL_MATCH_WEIGHT) / activeFilters.length) * 100;
}

export function checkIfFilterMatches(filter: TFormField, attempt: TExchangeAttempt, filters: TFormField[], fields: TFormField[]) {
...
export function checkIfFieldMatches(
	...
	const matchStatus = field.isMatch
		? field.isMatch(filter.value, field.value, filters, fields, attempt)
		: isEqual
		? TSpecStatus.Match
		: TSpecStatus.NoMatch;
	return matchStatus;
